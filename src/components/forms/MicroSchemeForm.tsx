...
export function MicroSchemeForm() {
  ...
  return (
    <fieldset className="space-y-3 rounded-lg border p-4">
      ...
      <NumberField label="iDeCo 個人掛金(月)" value={m.idecoMonthly} suffix="円" onChange={(v) => patch({ idecoMonthly: v })} hint="全額所得控除。第2号(企業年金なし)上限6.2万円(2026年12月施行・掛金は2027年1月分〜。2026年分の実質上限は2.3万円)" />
      <CheckField label="消費税を課す（個人事業主・簡易課税）" checked={m.consumptionTaxEnabled} onChange={(v) => patch({ consumptionTaxEnabled: v })} />
      <NumberField label="消費税 対売上率(%)" value={Math.round(m.consumptionTaxRate * 100)} step={1} suffix="%" onChange={(v) => patch({ consumptionTaxRate: v / 100 })} hint="簡易課税サービス業(第5種)＝売上5%。基準期間売上1000万円以下のインボイス登録者は2割特例で2%(令和8年分まで)も選択可" />
      <CheckField label="個人事業税を課す" checked={m.businessTaxEnabled} onChange={(v) => patch({ businessTaxEnabled: v })} />
      <NumberField label="個人事業税率(%)" value={Math.round(m.businessTaxRate * 100)} step={1} suffix="%" onChange={(v) => patch({ businessTaxRate: v / 100 })} hint="標準5%(請負業=第1種・コンサル=第3種)。事業主控除290万円後に課税。エンジニア等の準委任は法定70業種外で非課税の場合あり→その時はオフに" />
